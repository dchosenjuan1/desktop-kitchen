...
const CLASSIFICATION_CONFIG = {
  star: { label: 'Estrella', icon: Star, color: '#22c55e', bgColor: 'bg-green-500/10', textColor: 'text-green-400', borderColor: 'border-green-500/30', emoji: '⭐' },
  workhorse: { label: 'Caballo', icon: TrendingUp, color: '#3b82f6', bgColor: 'bg-blue-500/10', textColor: 'text-blue-400', borderColor: 'border-blue-500/30', emoji: '🐎' },
  puzzle: { label: 'Rompecabezas', icon: HelpCircle, color: '#f59e0b', bgColor: 'bg-amber-500/10', textColor: 'text-amber-400', borderColor: 'border-amber-500/30', emoji: '🧩' },
  dog: { label: 'Perro', icon: XCircle, color: '#ef4444', bgColor: 'bg-red-500/10', textColor: 'text-red-400', borderColor: 'border-red-500/30', emoji: '🐕' },
};
  ...
  return (
    <div className="bg-neutral-900 border border-neutral-700 rounded-lg p-3 shadow-xl">
      ...
      <div className="space-y-1 text-xs">
        <div className="flex justify-between gap-4"><span className="text-neutral-400">Precio:</span><span className="text-white">{fmt(item.price)}</span></div>
        <div className="flex justify-between gap-4"><span className="text-neutral-400">Costo:</span><span className="text-amber-400">{fmt(item.cogs_per_unit)}</span></div>
        <div className="flex justify-between gap-4"><span className="text-neutral-400">Margen:</span><span className="text-green-400">{fmt(item.contribution_margin)}</span></div>
        <div className="flex justify-between gap-4"><span className="text-neutral-400">Vendidos:</span><span className="text-white">{item.quantity_sold}</span></div>
        <div className="flex justify-between gap-4"><span className="text-neutral-400">Popularidad:</span><span className="text-white">{item.popularity_index}%</span></div>
        <div className="flex justify-between gap-4"><span className="text-neutral-400">Clasificación:</span><span className={config.textColor}>{config.emoji} {config.label}</span></div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-6">
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className={`text-sm font-medium ${config.textColor}`}>{config.label}s</span>
              </div>
              <p className="text-3xl font-bold text-white">{count}</p>
              <p className="text-neutral-500 text-xs mt-1">
                {summary.total_items > 0 ? Math.round((count / summary.total_items) * 100) : 0}% del menú
              </p>
            </button>
          );
        })}
      ...
      <div className="bg-neutral-900 p-4 rounded-lg border border-neutral-800 flex flex-wrap gap-6 text-sm">
        <div><span className="text-neutral-400">Ingreso total:</span> <span className="text-white font-bold">{fmt(summary.total_revenue)}</span></div>
        <div><span className="text-neutral-400">Contribución total:</span> <span className="text-green-400 font-bold">{fmt(summary.total_contribution)}</span></div>
        <div><span className="text-neutral-400">Margen promedio:</span> <span className="text-white font-bold">{fmt(summary.avg_contribution_margin)}</span></div>
        <div><span className="text-neutral-400">Productos analizados:</span> <span className="text-white font-bold">{summary.total_items}</span></div>
      </div>

      {/* BCG Matrix Scatter Plot */}
      <div className="bg-neutral-900 p-6 rounded-lg border border-neutral-800">
        <h3 className="text-xl font-bold text-white mb-1">Matriz de Ingeniería de Menú</h3>
        <p className="text-neutral-400 text-sm mb-4">Cada punto representa un producto. Eje X = popularidad, Eje Y = margen de contribución.</p>
        <ResponsiveContainer width="100%" height={400}>
          <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
            ...
            <XAxis
              ...
              unit="%"
              stroke="#737373"
              label={{ value: 'Popularidad (%)', position: 'bottom', fill: '#737373', fontSize: 12 }}
            />
            <YAxis
              type="number"
              ...
              stroke="#737373"
              tickFormatter={(v) => `$${v}`}
              label={{ value: 'Margen (MXN)', angle: -90, position: 'insideLeft', fill: '#737373', fontSize: 12 }}
            />
            ...
            <ReferenceLine
              ...
              stroke="#525252"
              strokeDasharray="5 5"
              label={{ value: 'Umbral popularidad', fill: '#525252', fontSize: 10, position: 'top' }}
            />
            <ReferenceLine
              y={summary.margin_threshold}
              stroke="#525252"
              strokeDasharray="5 5"
              label={{ value: 'Umbral margen', fill: '#525252', fontSize: 10, position: 'right' }}
            />
            {/* Quadrant labels */}
          ...
          <h3 className="text-xl font-bold text-white">
            ...
            {filterClass && (
              <span className={`ml-2 text-sm font-normal ${CLASSIFICATION_CONFIG[filterClass as keyof typeof CLASSIFICATION_CONFIG].textColor}`}>
                (filtrando: {CLASSIFICATION_CONFIG[filterClass as keyof typeof CLASSIFICATION_CONFIG].label}s)
              </span>
            )}
          </h3>
          <div className="flex items-center gap-2">
            <span className="text-neutral-400 text-xs">Ordenar:</span>
            <select
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value as any)}
              className="bg-neutral-800 text-white text-xs border border-neutral-700 rounded px-2 py-1"
            >
              <option value="total_contribution">Contribución total</option>
              <option value="contribution_margin">Margen unitario</option>
              <option value="popularity_index">Popularidad</option>
              <option value="quantity_sold">Unidades vendidas</option>
            </select>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-neutral-800 border-b border-neutral-700">
              <tr>
                <th className="text-left text-neutral-400 px-3 py-2 font-medium">Producto</th>
                <th className="text-left text-neutral-400 px-3 py-2 font-medium">Categoría</th>
                <th className="text-center text-neutral-400 px-3 py-2 font-medium">Clasificación</th>
                <th className="text-right text-neutral-400 px-3 py-2 font-medium">Precio</th>
                <th className="text-right text-neutral-400 px-3 py-2 font-medium">Costo</th>
                <th className="text-right text-neutral-400 px-3 py-2 font-medium">Margen</th>
                <th className="text-right text-neutral-400 px-3 py-2 font-medium">Margen %</th>
                <th className="text-right text-neutral-400 px-3 py-2 font-medium">Vendidos</th>
                <th className="text-right text-neutral-400 px-3 py-2 font-medium">Popularidad</th>
                <th className="text-right text-neutral-400 px-3 py-2 font-medium">Contribución</th>
              </tr>
            </thead>
            <tbody>
